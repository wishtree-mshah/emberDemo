import { templateFactory, PartialDefinitionImpl, EMPTY_BLOCKS, NONE, staticComponent, UNHANDLED, syntaxCompilationContext } from '@glimmer/opcode-compiler';
import { get, PROPERTY_DID_CHANGE, set, computed, alias, CUSTOM_TAG_FOR, tagForObject, objectAt, tagForProperty, _getProp } from '@ember/-internals/metal';
import { getOwner, setOwner } from '@ember/-internals/owner';
import { TargetActionSupport, FrameworkObject, _contentFor, isArray } from '@ember/-internals/runtime';
import { enumerableSymbol, symbol, HAS_NATIVE_PROXY, getDebugName, guidFor, isProxy, HAS_NATIVE_SYMBOL, isEmberArray, isObject, uuid, toString } from '@ember/-internals/utils';
...
import { isUpdatableRef, updateRef, valueForRef, createConstRef, isConstRef, createComputeRef, UNDEFINED_REFERENCE, childRefFor, childRefFromParts, createPrimitiveRef, createUnboundRef, isInvokableRef, NULL_REFERENCE, createInvokableRef, createReadOnlyRef, createDebugAliasRef } from '@glimmer/reference';
import { normalizeProperty, registerDestructor, associateDestroyableChild, EMPTY_ARGS, EMPTY_NAMED, EMPTY_POSITIONAL, isDestroyed, isDestroying, reifyPositional, reifyArgs, reifyNamed, createCapturedArgs, curry, getDynamicVar, clientBuilder, destroy, DOMChanges, DOMTreeConstruction, inTransaction, renderMain, runtimeContext, rehydrationBuilder } from '@glimmer/runtime';
export { DOMChanges, DOMTreeConstruction, isSerializationFirstNode } from '@glimmer/runtime';
import { createTag, dirtyTag, track, createCache, getValue, consumeTag, deprecateMutationsInTrackingTransaction, beginUntrackFrame, endUntrackFrame, valueForTag, logTrackingStack, beginTrackFrame, endTrackFrame, validateTag, CONSTANT_TAG, isTracking, tagFor, setTrackingTransactionEnv, createUpdatableTag, untrack, CURRENT_TAG } from '@glimmer/validator';
import { hasDOM } from '@ember/-internals/browser-environment';
import { flaggedInstrument, _instrumentStart } from '@ember/instrumentation';
import { inject } from '@ember/service';
import { ENV } from '@ember/-internals/environment';
import { _WeakSet, unwrapTemplate, EMPTY_ARRAY, expect, Stack, dict, unwrapHandle } from '@glimmer/util';
import { COMPONENT_MANAGER_STRING_LOOKUP, EMBER_COMPONENT_IS_VISIBLE, PARTIALS } from '@ember/deprecated-features';
...
import setGlobalContext, { toBool } from '@glimmer/global-context';
...
function getModifierManager(owner, definition) {
  ...
  if (factory !== undefined) {
    ...
    assert(`Custom modifier managers must have a \`capabilities\` property that is the result of calling the \`capabilities('3.13' | '3.22')\` (imported via \`import { capabilities } from '@ember/modifier';\`). Received: \`${JSON.stringify(manager.capabilities)}\` for: \`${manager}\``, FROM_CAPABILITIES.has(manager.capabilities));
    return manager;
  }
...
function getHelperManager(owner, definition) {
  ...
  if (factory !== undefined) {
    let manager = getManagerInstanceForOwner(owner, factory);
    assert(`Custom helper managers must have a \`capabilities\` property that is the result of calling the \`capabilities('3.23')\` (imported via \`import { capabilities } from '@ember/helper';\`). Received: \`${JSON.stringify(manager.capabilities)}\` for: \`${manager}\``, FROM_CAPABILITIES.has(manager.capabilities));
    return manager;
  }
...
function helperCapabilities(managerAPI, options = {}) {
  ...
  assert('You must pass either the `hasValue` OR the `hasScheduledEffect` capability when defining a helper manager. Passing neither, or both, is not permitted.', (options.hasValue || options.hasScheduledEffect) && !(options.hasValue && options.hasScheduledEffect));
  assert('The `hasScheduledEffect` capability has not yet been implemented for helper managers. Please pass `hasValue` instead', !options.hasScheduledEffect);
...
function invokeHelper(context, definition, computeArgs) {
  assert(`Expected a context object to be passed as the first parameter to invokeHelper, got ${context}`, context !== null && typeof context === 'object');
  const owner = getOwner(context);
  const manager = getHelperManager(owner, definition); // TODO: figure out why assert isn't using the TS assert thing

  assert(`Expected a helper definition to be passed as the second parameter to invokeHelper, but no helper manager was found. The definition value that was passed was \`${getDebugName(definition)}\`. Did you use setHelperManager to associate a helper manager with this value?`, manager);
  ...
  if (hasValue(manager)) {
    cache = createCache(() => {
      assert(`You attempted to get the value of a helper after the helper was destroyed, which is not allowed`, !isDestroying(cache) && !isDestroyed(cache));
...
const CLASSIC_HELPER_MANAGERS = new _WeakSet();
...
function applyAttributeBindings(attributeBindings, component, rootRef, operations) {
  ...
  if (seen.indexOf('id') === -1) {
    ...
    operations.setAttribute('id', createPrimitiveRef(id$$1), false, null);
  }
...
class CurlyComponentManager extends AbstractManager {
  ...
  create(environment, state, args, dynamicScope, callerSelfRef, hasBlock) {
    ...
    props._target = valueForRef(callerSelfRef); // static layout asserts CurriedDefinition
...
class DebugRenderTree {
  ...
  create(state, node) {
    let internalNode = Object.assign(Object.assign({}, node), {
      bounds: null,
      refs: new Set()
    });
...
if (DEBUG) {
  helper$1 = args => {
    ...
    return createComputeRef(() => {
      let value = valueForRef(inner);
      assert(valueForRef(messageRef), typeof value !== 'string');
      return value;
    });
  };
} else {
...
function concat (args) {
  let captured = args.positional.capture();
  return createComputeRef(() => reifyPositional(captured).map(normalizeTextValue).join(''), null, 'concat');
}
...
function mut (args) {
  let ref = args.positional.at(0); // TODO: Improve this error message. This covers at least two distinct
  // cases:
  //
...
class OnModifierState {
  ...
  updateFromArgs() {
    ...
    assert('You must pass a valid DOM event name as the first argument to the `on` modifier', args.positional[0] !== undefined && typeof valueForRef(args.positional[0]) === 'string');
...
class RuntimeResolverImpl {
  ...
  _lookupHelper(_name, meta) {
    ...
    assert('helper managers have not been enabled yet, you must use classic helpers', true
    /* EMBER_GLIMMER_HELPER_MANAGER */
    || isClassicHelperManager(manager) || manager === SIMPLE_CLASSIC_HELPER_MANAGER); // For classic class based helpers, we need to pass the factoryFor result itself rather
    ...
    if (pair.component !== null) {
      ...
      if (manager !== undefined) {
        if (isInternalManager(manager)) {
          ...
          definition = new CustomManagerDefinition(name, pair.component, manager, layout !== undefined ? layout : owner.lookup(privatize`template:components/-default`)(owner));
        }
      }
    }
...
class RootState {
  constructor(root, runtime, context, template, self, parentElement, dynamicScope, builder) {
    ...
    this.render = errorLoopTransaction(() => {
      ...
      let iterator = renderMain(runtime, context, self, builder(runtime.env, {
...
class Renderer {
  constructor(owner, document, env, rootTemplate, viewRegistry, destinedForDOM = false, builder = clientBuilder) {
    ...
    let runtimeResolver = this._runtimeResolver = new RuntimeResolverImpl(owner, env.isInteractive);
...
export { RootTemplate, template, counters as templateCacheCounters, Checkbox, TextField, TextArea, LinkComponent, Component, Helper, helper, SafeString, escapeExpression, htmlSafe, isHTMLSafe, Renderer, InertRenderer, InteractiveRenderer, _resetRenderers, renderSettled, getTemplate, setTemplate, hasTemplate, getTemplates, setTemplates, setupEngineRegistry, setupApplicationRegistry, registerMacros as _registerMacros, experimentalMacros as _experimentalMacros, AbstractManager as AbstractComponentManager, INVOKE, OutletView, setComponentManager, setModifierManager, setHelperManager, capabilities, capabilities$1 as modifierCapabilities, helperCapabilities, invokeHelper, setComponentTemplate, getComponentTemplate };
